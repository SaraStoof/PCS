...
def particle_loop(grid, ATTACH_PROB, DECAY_PROB, batch_size=1000):
    ...
    for i in prange(TIMESTEPS):
        if i % TIMESTEPS_PER_DAY == 0:
            ...
            m_history_3d[i //
                         TIMESTEPS_PER_DAY] = coverage_to_m_value(mold_coverage(grid))
...
def monte_carlo(ATTACH_PROB, DECAY_PROB):
    ...
    return history_3d, history_surf


def main():
    global TEMP, RH
    global ATTACH_PROB, DECAY_PROB
    ...
    for i in range(len(temp_list)):
        ...
        for rh in rh_list:
            ...
            history_3d, history_surf = monte_carlo(ATTACH_PROB, DECAY_PROB)
