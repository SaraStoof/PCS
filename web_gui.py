"""
A simple web GUI for the 3D mold growth simulation.
Run with `python web_gui.py` and visit http://.../ in your web browser, as prompted.

Dependenices: dash, plotly, numpy, dla_3d_for_gui

Tip: If port 8050 is already in use, use `lsof -i :8050` to find the PID and
...
app.layout = html.Div([
    ...
    html.Div([
        # Left side - Graph
        html.Div([
            dcc.Graph(id="graph", style={"height": "80vh"})  # Adjust height as needed
        ...
        html.Div([
            ...
            dcc.Input(id="x-coord", type="number", min=1, placeholder="Enter an integer", value=50),
            dcc.Input(id="y-coord", type="number", min=1, placeholder="Enter an integer", value=50),
            dcc.Input(id="z-coord", type="number", min=1, placeholder="Enter an integer", value=100),
            ...
            html.P("Temperature:"),
            dcc.Input(id="temp", type="number", min=1, placeholder="Enter an integer", value=30),
...
def update_bar_chart(n_clicks, grid_x, grid_y, grid_z, spawn_x, spawn_y, spawn_z,
    ...
    if any(v is None or v <= 0 for v in [grid_x, grid_y, grid_z, num_sims, days, temp, rh]):
    ...
    fig = px.scatter_3d(
        x=x, y=y, z=z,
        color=color,
        color_continuous_scale='Viridis' #'Aggrnyl' or 'Viridis'
    )
    ...
    if "on" in gradient_toggle:
        ...
        fig.update_traces(marker=dict(size=2, color='green'))
